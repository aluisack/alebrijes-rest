...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({ alebrijes });
}
...
export async function POST(req: NextRequest) {
  ...
  const baseSlug = data.nombre.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");
  let slug = baseSlug;
  let count = 0;
  ...
  const alebrije = await prisma.alebrije.create({
    data: { ...data, slug, artesanoId: session.user.id, publicado: false },
  });
